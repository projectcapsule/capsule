...
func slowIsOwner(o rbac.OwnerStatusListSpec, name string, groups []string) bool {
	for _, owner := range o {
		switch owner.Kind {
...
func linearFind(o rbac.OwnerStatusListSpec, name string, kind rbac.OwnerKind) (rbac.CoreOwnerSpec, bool) {
	for _, x := range o {
		if x.Kind == kind && x.Name == name {
			return x, true
		}
	}
	return rbac.CoreOwnerSpec{}, false
}
...
func TestUpsert_AddsNewOwnerToEmptyList(t *testing.T) {
	...
	got := list[0]
	if got.Kind != rbac.UserOwner || got.Name != "alice" {
...
func TestUpsert_KeepsListSortedAndMergesIntoExistingInUnsortedInitialSlice(t *testing.T) {
	...
	sorted := make(rbac.OwnerStatusListSpec, len(list))
	copy(sorted, list)
	sort.Sort(rbac.GetByKindAndName(sorted))
...
func TestGetByKindAndNameOrdering(t *testing.T) {
	...
	got := append(rbac.OwnerStatusListSpec(nil), o...)
	sort.Sort(rbac.GetByKindAndName(got))

	// Manually sorted expectation using the same logic.
	want := append(rbac.OwnerStatusListSpec(nil), o...)
...
func TestFindOwner_Randomized(t *testing.T) {
	...
	for listIdx := 0; listIdx < numLists; listIdx++ {
		...
		for lookupIdx := 0; lookupIdx < numLookupsPerList; lookupIdx++ {
			...
			listCopy := append(rbac.OwnerStatusListSpec(nil), list...)
