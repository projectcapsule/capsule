...
func (h *patchHandler) OnCreate(client client.Client, decoder admission.Decoder, recorder record.EventRecorder) capsulewebhook.Func {
	return func(ctx context.Context, req admission.Request) *admission.Response {
		return h.patchCreate(ctx, req, client, decoder, recorder)
	}
}

func (h *patchHandler) OnDelete(client.Client, admission.Decoder, record.EventRecorder) capsulewebhook.Func {
	return func(context.Context, admission.Request) *admission.Response {
		return nil
	}
}

func (h *patchHandler) OnUpdate(c client.Client, decoder admission.Decoder, recorder record.EventRecorder) capsulewebhook.Func {
	return func(ctx context.Context, req admission.Request) *admission.Response {
...
func (h *patchHandler) namespaceIsOwned(ns *corev1.Namespace, tenantList *capsulev1beta2.TenantList, req admission.Request) bool {
...
func (h *patchHandler) patchCreate(ctx context.Context, req admission.Request, client client.Client, decoder admission.Decoder, recorder record.EventRecorder) *admission.Response {
	ns := &corev1.Namespace{}
	...
	if label, ok := ns.ObjectMeta.Labels[ln]; ok {
		...
		response := h.patchResponse(tnt, ns, recorder)
	...
	if len(tenants) == 1 {
		...
		response := h.patchResponse(&tenants[0], ns, recorder)

		return &response
	}

	if h.configuration.ForceTenantPrefix() {
		for _, tnt := range tenants {
			if strings.HasPrefix(ns.GetName(), fmt.Sprintf("%s-", tnt.GetName())) {
				response := h.patchResponse(tnt.DeepCopy(), ns, recorder)
...
func (h *patchHandler) patchResponse(tenant *capsulev1beta2.Tenant, ns *corev1.Namespace, recorder record.EventRecorder) admission.Response {
...
func (h *patchHandler) listTenantsForOwnerKind(ctx context.Context, ownerKind string, ownerName string, clt client.Client) (*capsulev1beta2.TenantList, error) {
	tntList := &capsulev1beta2.TenantList{}
...
func (h *patchHandler) validateNamespacePrefix(ns *corev1.Namespace, tenant *capsulev1beta2.Tenant) *admission.Response {
