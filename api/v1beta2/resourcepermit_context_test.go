...
func TestResourcePermitLoadsParameterTemplatedContextForAllItems(t *testing.T) {
	t.Parallel()
	...
	for i, item := range items {
		...
		if !strings.Contains(rendered, "temporary-permit-") || !strings.Contains(rendered, "production") {
			t.Fatalf("rendered item %d does not contain parameter and loaded context: %s", i, rendered)
		}
	}
}

func TestResourcePermitRendersTargetsAndStructuredMultiYAMLTemplate(t *testing.T) {
	t.Parallel()

	br := &ResourcePermit{Spec: ResourcePermitSpec{Params: &runtime.RawExtension{Raw: []byte(`{"name":"temporary-permit"}`)}}}
...
func TestResourcePermitRendersTrustedRequestContext(t *testing.T) {
	t.Parallel()
...
func TestResourcePermitRequestContextKeyIsReserved(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			br := &ResourcePermit{Spec: ResourcePermitSpec{Params: tt.params}}
...
func TestResourcePermitRendersRequesterRoleBindingCartesianProduct(t *testing.T) {
	t.Parallel()
...
kind: RoleBinding
metadata:
  name: 'resourcepermit-{{ deterministicUUID $.request.username $.request.timestamp $namespace $clusterRole | toLower }}'
  namespace: '{{ $namespace }}'
roleRef:
...
func TestResourcePermitContextCannotReplaceParameter(t *testing.T) {
	t.Parallel()

	br := &ResourcePermit{Spec: ResourcePermitSpec{Params: &runtime.RawExtension{Raw: []byte(`{"name":"request-value"}`)}}}
	_, err := br.RenderResources(
