...
func TestGlobalResourceLeaseTemplateApprovalCondition(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "empty condition", want: true},
		{
			...
			request:    ResourceLease{Spec: ResourceLeaseSpec{Reason: "incident"}},
			want:       true,
		},
		{
			...
			request: ResourceLease{Spec: ResourceLeaseSpec{
				Reason:    "incident",
		...
		},
		{
			...
			request: ResourceLease{Spec: ResourceLeaseSpec{Requestor: resourcelease.AccessEntity{
		...
		},
		{
			...
			request: ResourceLease{Status: ResourceLeaseStatus{Review: &ReviewInfo{Reviewer: &resourcelease.AccessEntity{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			brt := &GlobalResourceLeaseTemplate{Spec: GlobalResourceLeaseTemplateSpec{
				Approvals: resourcelease.ApprovalSpec{Conditions: tt.conditions},
			}}
...
func TestResourceLeaseUsesApprovalSnapshot(t *testing.T) {
	t.Parallel()

	template := &GlobalResourceLeaseTemplate{Spec: GlobalResourceLeaseTemplateSpec{
		Approvals: resourcelease.ApprovalSpec{Conditions: []string{"false"}},
	}}
	request := &ResourceLease{
		Status: ResourceLeaseStatus{Request: &ResourceLeaseStatusRequest{
			Approvals: &resourcelease.ApprovalSpec{Conditions: []string{"true"}},
		}},
	}
