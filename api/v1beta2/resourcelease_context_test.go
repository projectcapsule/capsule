...
func TestResourceLeaseLoadsParameterTemplatedContextForAllItems(t *testing.T) {
	t.Parallel()
	...
	br := &ResourceLease{
		...
		Spec: ResourceLeaseSpec{Params: &runtime.RawExtension{Raw: []byte(`{
	...
	for i, item := range items {
		...
		if !strings.Contains(rendered, "temporary-lease-") || !strings.Contains(rendered, "production") {
			t.Fatalf("rendered item %d does not contain parameter and loaded context: %s", i, rendered)
		}
	}
}

func TestResourceLeaseRendersTargetsAndStructuredMultiYAMLTemplate(t *testing.T) {
	t.Parallel()

	br := &ResourceLease{Spec: ResourceLeaseSpec{Params: &runtime.RawExtension{Raw: []byte(`{"name":"temporary-lease"}`)}}}
...
func TestResourceLeaseRendersTrustedRequestContext(t *testing.T) {
	t.Parallel()

	created := metav1.NewTime(time.Date(2026, time.September, 1, 8, 30, 0, 0, time.UTC))
	br := &ResourceLease{
		ObjectMeta: metav1.ObjectMeta{Name: "temporary-lease", CreationTimestamp: created},
		Spec: ResourceLeaseSpec{Requestor: resourcelease.AccessEntity{
			Name:   "alice",
...
func TestResourceLeaseRequestContextKeyIsReserved(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			br := &ResourceLease{Spec: ResourceLeaseSpec{Params: tt.params}}
...
func TestResourceLeaseRendersRequesterRoleBindingCartesianProduct(t *testing.T) {
	t.Parallel()
	...
	br := &ResourceLease{
		...
		Spec: ResourceLeaseSpec{
			Requestor: resourcelease.AccessEntity{Name: "alice"},
...
kind: RoleBinding
metadata:
  name: 'resourcelease-{{ deterministicUUID $.request.username $.request.timestamp $namespace $clusterRole | toLower }}'
  namespace: '{{ $namespace }}'
roleRef:
...
func TestResourceLeaseContextCannotReplaceParameter(t *testing.T) {
	t.Parallel()

	br := &ResourceLease{Spec: ResourceLeaseSpec{Params: &runtime.RawExtension{Raw: []byte(`{"name":"request-value"}`)}}}
	_, err := br.RenderResources(
