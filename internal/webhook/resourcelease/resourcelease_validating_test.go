...
type resourceLeaseValidationTestConfiguration struct {
	runtimeconfiguration.Configuration
	administrators rbac.UserListSpec
}

func (c resourceLeaseValidationTestConfiguration) Administrators() rbac.UserListSpec {
	return c.administrators
}

func TestResourceLeaseValidationHandler(t *testing.T) {
	defaultTemplateName := "foo"
	alternateTemplateName := "bar"
	ctx := context.Background()
	log := ctrl.Log.WithName("test")
	templateRef := func(name string) capsulev1beta2.GlobalResourceLeaseTemplateReference {
		return capsulev1beta2.GlobalResourceLeaseTemplateReference{
			Kind: capsulev1beta2.GlobalResourceLeaseTemplateKind,
			Name: name,
		}
	}
	localTemplateRef := func(name string) capsulev1beta2.ResourceLeaseTemplateReference {
		return capsulev1beta2.ResourceLeaseTemplateReference{
	...
	t.Run("OnCreate", func(t *testing.T) {
		tests := []struct {
				...
				setup: func(reader *mc.MockReader) {
					reader.EXPECT().
						Get(
							...
							gm.AssignableToTypeOf(&capsulev1beta2.ResourceLeaseTemplate{}),
						).
						Return(nil)
				},
				expected: 0,
			...
			},
			{
				...
				br: &capsulev1beta2.ResourceLease{
					Spec: capsulev1beta2.ResourceLeaseSpec{
						Template:  templateRef(defaultTemplateName),
						Requestor: resourcelease.AccessEntity{Name: "bob"},
					},
				},
				setup: func(reader *mc.MockReader) {
					reader.EXPECT().
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				decoder := &test.Decoder[*capsulev1beta2.ResourceLease]{
					Object: tt.br,
				}
				validator := ResourceLeaseValidationHandler(log, nil)
	...
	t.Run("OnDelete", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				br: &capsulev1beta2.ResourceLease{Status: capsulev1beta2.ResourceLeaseStatus{
					Phase: capsulev1beta2.ResourceLeasePhaseCreated,
				}},
			},
			{
			...
			},
			{
				...
				br: &capsulev1beta2.ResourceLease{Status: capsulev1beta2.ResourceLeaseStatus{
					Phase: capsulev1beta2.ResourceLeasePhasePending,
				}},
			},
			{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				decoder := &test.Decoder[*capsulev1beta2.ResourceLease]{OldObject: tt.br}
				validator := ResourceLeaseValidationHandler(log, resourceLeaseValidationTestConfiguration{
	...
	t.Run("OnUpdate", func(t *testing.T) {
		tests := []struct {
			...
			},
			{
				...
				expected: http.StatusForbidden,
				errMsg:   "template cannot be changed. old: GlobalResourceLeaseTemplate/foo, new: GlobalResourceLeaseTemplate/bar",
			},
			{
			...
			},
			{
				...
				oldBr: &capsulev1beta2.ResourceLease{
					Status: capsulev1beta2.ResourceLeaseStatus{
						Request: &capsulev1beta2.ResourceLeaseStatusRequest{Impersonation: &capsulemeta.NamespacedRFC1123ObjectReferenceWithNamespace{
			...
			},
			{
				...
				oldBr: &capsulev1beta2.ResourceLease{
					Status: capsulev1beta2.ResourceLeaseStatus{
						Request: &capsulev1beta2.ResourceLeaseStatusRequest{Template: &capsulev1beta2.ResolvedResourceLeaseTemplateReference{
							ResourceLeaseTemplateReference: templateRef(defaultTemplateName),
			...
			},
			{
				...
				oldBr: &capsulev1beta2.ResourceLease{Status: capsulev1beta2.ResourceLeaseStatus{
					Request: &capsulev1beta2.ResourceLeaseStatusRequest{Approvals: &resourcelease.ApprovalSpec{
				...
				oldBr: &capsulev1beta2.ResourceLease{
					Status: capsulev1beta2.ResourceLeaseStatus{
						...
						Review: &capsulev1beta2.ReviewInfo{
							Verdict: capsulev1beta2.ResourceLeaseVerdictPending,
						},
					},
				},
				newBr: &capsulev1beta2.ResourceLease{
					Status: capsulev1beta2.ResourceLeaseStatus{
						Phase: capsulev1beta2.ResourceLeasePhaseRequested,
						Review: &capsulev1beta2.ReviewInfo{
							Verdict: capsulev1beta2.ResourceLeaseVerdictApproved,
						},
					},
				},
				request:  admission.Request{AdmissionRequest: admissionv1.AdmissionRequest{SubResource: "status"}},
				expected: http.StatusForbidden,
				errMsg:   "ResourceLease status can only be changed by requesting a lifecycle transition",
			},
			{
				name: "deny approval while rendering is not ready",
				oldBr: &capsulev1beta2.ResourceLease{
					Spec: capsulev1beta2.ResourceLeaseSpec{Template: templateRef(defaultTemplateName)},
					Status: capsulev1beta2.ResourceLeaseStatus{
				...
				request:  admission.Request{AdmissionRequest: admissionv1.AdmissionRequest{SubResource: "status"}},
				expected: http.StatusForbidden,
				errMsg:   "cannot approve ResourceLease: rendered resources are not ready: rendering resource 0 failed",
			},
			{
				...
				newBr: &capsulev1beta2.ResourceLease{
					...
					Status: capsulev1beta2.ResourceLeaseStatus{
						...
						Review: &capsulev1beta2.ReviewInfo{Reviewer: &resourcelease.AccessEntity{
							Name:   "alice",
...
func TestResourceLeaseValidationRejectsInvalidParametersBeforeRendering(t *testing.T) {
	t.Parallel()
	...
	validator := ResourceLeaseValidationHandler(ctrl.Log.WithName("test"), nil)
