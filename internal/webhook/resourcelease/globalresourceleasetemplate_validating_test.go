...
func TestGlobalResourceLeaseTemplateValidationHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			brt: &capsulev1beta2.GlobalResourceLeaseTemplate{
				Spec: capsulev1beta2.GlobalResourceLeaseTemplateSpec{
					Approvals: resourcelease.ApprovalSpec{Approvers: rbac.UserListSpec{{Kind: rbac.UserOwner}}},
					Resources: []apiruntime.ResourceTemplate{{Targets: []runtime.RawExtension{{Object: &corev1.ConfigMap{}}}}},
				},
			},
			expected: http.StatusForbidden,
		...
		},
		{
			...
			brt: &capsulev1beta2.GlobalResourceLeaseTemplate{Spec: capsulev1beta2.GlobalResourceLeaseTemplateSpec{
...
`}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			decoder := &test.Decoder[*capsulev1beta2.GlobalResourceLeaseTemplate]{
				Object: tt.brt,
			}
			validator := GlobalResourceLeaseTemplateValidationHandler(log)
