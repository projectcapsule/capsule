...
func (h *resourcePermitResourceHandler) handle(
	...
	obj, err := resourcePermitProtectionObject(req, decoder, preferOld)
	if err != nil {
		return ad.ErroredResponse(err)
	}

	if obj != nil && obj.GetAnnotations()[meta.ResourcePermitServiceAccountAnnotation] == req.UserInfo.Username {
		return nil
	}

	return ad.Deny(
		"resources protected by a ResourcePermit can only be changed by the Capsule controller or the template ServiceAccount",
	)
}

func resourcePermitProtectionObject(
	req admission.Request,
	decoder admission.Decoder,
	preferOld bool,
