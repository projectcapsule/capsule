...
func TestResourceLeaseResourceHandler(t *testing.T) {
	...
	handler := ResourceLeaseResourceHandler()
	...
	t.Run("template ServiceAccount may update its protected resource", func(t *testing.T) {
		oldObj := protectedResourceLeaseResource("system:serviceaccount:operations:runner")
		newObj := oldObj.DeepCopy()
		newObj.SetAnnotations(map[string]string{
			meta.ResourceLeaseServiceAccountAnnotation: "system:serviceaccount:attacker:runner",
		})
	...
	t.Run("template ServiceAccount may adopt and protect a resource", func(t *testing.T) {
		...
		newObj := protectedResourceLeaseResource("system:serviceaccount:operations:runner")
		decoder := &webhooktest.Decoder[*unstructured.Unstructured]{Object: newObj, OldObject: oldObj}
...
func protectedResourceLeaseResource(serviceAccount string) *unstructured.Unstructured {
	obj := &unstructured.Unstructured{}
	obj.SetLabels(map[string]string{
		meta.ProtectedByCapsuleLabel: meta.ValueControllerResourceLease,
	})
	obj.SetAnnotations(map[string]string{
		meta.ResourceLeaseServiceAccountAnnotation: serviceAccount,
	})
