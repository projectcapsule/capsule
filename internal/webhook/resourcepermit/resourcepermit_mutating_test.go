...
func TestResourcePermitMutationHandlerOnCreate(t *testing.T) {
	...
	tests := []struct {
		...
		username   string
		groups     []string
		entityType resourcepermitapi.AccessEntityType
	}{
		{
			name:       "user",
			username:   "alice",
			groups:     []string{"developers", "on-call"},
			entityType: resourcepermitapi.AccessEntityTypeUser,
		},
		{
			name:       "service account",
			username:   "system:serviceaccount:team-a:reviewer",
			groups:     []string{"system:serviceaccounts", "system:serviceaccounts:team-a"},
			entityType: resourcepermitapi.AccessEntityTypeServiceAccount,
		},
	}
...
func TestResourcePermitMutationHandlerOnApproval(t *testing.T) {
	...
	tests := []struct {
		...
		wantReviewer resourcepermitapi.AccessEntity
		wantPatch    bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantPatch {
				...
				mutated := applyResponsePatches(t, raw, resp)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseApproved, mutated.Status.Phase)
				...
				assert.Equal(t, capsulev1beta2.ResourcePermitVerdictApproved, mutated.Status.Review.Verdict)
			} else {
				assert.Nil(t, resp)
...
func TestResourcePermitMutationHandlerIgnoresApprovalOutsideStatusSubresource(t *testing.T) {
	t.Parallel()
...
func TestResourcePermitMutationHandlerAppliesApprovalFromPhaseOnly(t *testing.T) {
	t.Parallel()
	...
	oldBr := &capsulev1beta2.ResourcePermit{
		Spec: capsulev1beta2.ResourcePermitSpec{Template: capsulev1beta2.GlobalResourcePermitTemplateReference{
			Kind: capsulev1beta2.GlobalResourcePermitTemplateKind,
			Name: "template",
		}},
	...
	newBr := &capsulev1beta2.ResourcePermit{
		Spec:   oldBr.Spec,
		Status: capsulev1beta2.ResourcePermitStatus{Phase: capsulev1beta2.ResourcePermitPhaseApproved},
	}
	raw, err := json.Marshal(newBr)
	require.NoError(t, err)
	...
	assert.Equal(t, capsulev1beta2.ResourcePermitVerdictApproved, mutated.Status.Review.Verdict)
	approved := mutated.LatestTransition(capsulev1beta2.ResourcePermitPhaseApproved)
	require.NotNil(t, approved)
	assert.Equal(t, "ApprovedByUser", approved.Reason)
	assert.Equal(t, "Resource permit approved", approved.Message)
	assert.Equal(t, "alice", approved.Actor.Name)
	assert.Equal(t, capsulev1beta2.ResourcePermitPhaseApproved, approved.Type)
}

func TestResourcePermitMutationHandlerAppliesExpirationFromStoredStatus(t *testing.T) {
	t.Parallel()

	keepFor := resourcepermitapi.ExtendedDuration(10 * time.Minute)
	resources := []apiruntime.RenderedResource{{
		Targets: []runtime.RawExtension{{Raw: []byte(`{"apiVersion":"v1","kind":"ConfigMap"}`)}},
	}}
	serviceAccount := &apimeta.NamespacedRFC1123ObjectReferenceWithNamespace{
	...
	mutated := applyResponsePatches(t, raw, resp)
	assert.Equal(t, capsulev1beta2.ResourcePermitPhaseExpired, mutated.Status.Phase)
	assert.Equal(t, resources, mutated.Status.Request.Resources)
	assert.Equal(t, serviceAccount, mutated.Status.Request.Impersonation)
	assert.Equal(t, template, mutated.Status.Request.Template)
	require.NotNil(t, mutated.Status.KeepUntil)
	assert.True(t, mutated.Status.KeepUntil.After(time.Now()))
	expired := mutated.LatestTransition(capsulev1beta2.ResourcePermitPhaseExpired)
	require.NotNil(t, expired)
	assert.Equal(t, "ExpiredByUser", expired.Reason)
	assert.Equal(t, "Resource permit expired by alice", expired.Message)
	assert.Equal(t, "alice", expired.Actor.Name)
	assert.Equal(t, capsulev1beta2.ResourcePermitPhaseExpired, expired.Type)
	...
	validationDecoder := &test.Decoder[*capsulev1beta2.ResourcePermit]{
		Object:    mutated,
		OldObject: oldBr,
	}
	validationResponse := ResourcePermitValidationHandler(log.Log.WithName("test"), nil).OnUpdate(
		nil,
		nil,
		validationDecoder,
		nil,
...
func TestResourcePermitMutationHandlerAppliesRequesterRetryFromStoredFailure(t *testing.T) {
	t.Parallel()
	...
	oldBr := &capsulev1beta2.ResourcePermit{
		...
		Status: capsulev1beta2.ResourcePermitStatus{
			...
			Request: &capsulev1beta2.ResourcePermitStatusRequest{Resources: resources},
		},
	}
	newBr := oldBr.DeepCopy()
	newBr.Status = capsulev1beta2.ResourcePermitStatus{Phase: capsulev1beta2.ResourcePermitPhaseRetrying}
	raw, err := json.Marshal(newBr)
	require.NoError(t, err)
	...
	mutated := applyResponsePatches(t, raw, resp)
	assert.Equal(t, capsulev1beta2.ResourcePermitPhaseRetrying, mutated.Status.Phase)
	assert.Equal(t, oldBr.Status.Failure, mutated.Status.Failure)
	assert.Equal(t, resources, mutated.Status.Request.Resources)
	retryTransition := mutated.LatestTransition(capsulev1beta2.ResourcePermitPhaseRetrying)
	require.NotNil(t, retryTransition)
	assert.Equal(t, "RetryPermitedByUser", retryTransition.Reason)
	assert.Contains(t, retryTransition.Message, "alice")
	assert.Equal(t, "alice", retryTransition.Actor.Name)
}

func TestResourcePermitMutationHandlerRejectsInvalidTransition(t *testing.T) {
	t.Parallel()
	...
	newBr := oldBr.DeepCopy()
	newBr.Status.Phase = capsulev1beta2.ResourcePermitPhaseActive
	raw, err := json.Marshal(newBr)
	require.NoError(t, err)
...
func applyResponsePatches(
	...
	mutated := &capsulev1beta2.ResourcePermit{}
