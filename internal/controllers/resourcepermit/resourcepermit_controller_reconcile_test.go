...
func TestResourcePermitReconciler_reconcile(t *testing.T) {
	s := scheme.Scheme
	_ = capsulev1beta2.AddToScheme(s)

	matchBr := gm.AssignableToTypeOf(&capsulev1beta2.ResourcePermit{})
	matchBrt := gm.AssignableToTypeOf(&capsulev1beta2.GlobalResourcePermitTemplate{})
	matchLocalBrt := gm.AssignableToTypeOf(&capsulev1beta2.ResourcePermitTemplate{})
	...
	tests := []struct {
	...
	}{
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				...
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseRequested, br.Status.Phase)
				require.Len(t, br.Status.Transitions, 2)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseCreated, br.Status.Transitions[0].Type)
				...
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseRequested, br.Status.Transitions[1].Type)
				require.NotNil(t, br.Status.Request.Template)
				assert.Equal(t, capsulev1beta2.GlobalResourcePermitTemplateKind, br.Status.Request.Template.Kind)
		...
		},
		{
			...
			mocks: func(cl *mc.MockClient, scl *mc.MockSubResourceWriter) {
				...
				cl.EXPECT().
					...
					Do(func(_ any, _ any, brt *capsulev1beta2.ResourcePermitTemplate, _ ...any) {
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				require.NotNil(t, br.Status.Request.Template)
				assert.Equal(t, capsulev1beta2.ResourcePermitTemplateKind, br.Status.Request.Template.Kind)
				assert.Equal(t, templateName, br.Status.Request.Template.Name)
		...
		},
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseCreated, br.Status.Phase)
		...
		},
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				...
				assert.Equal(t, capsulev1beta2.ResourcePermitFailureStagePreflight, br.Status.Failure.Stage)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseRequested, br.Status.Failure.RetryPhase)
			...
			br: &capsulev1beta2.ResourcePermit{
				...
				Status: capsulev1beta2.ResourcePermitStatus{
					...
					Failure: &capsulev1beta2.ResourcePermitFailure{
						Stage:      capsulev1beta2.ResourcePermitFailureStagePreflight,
		...
		},
		{
			...
			br: &capsulev1beta2.ResourcePermit{
				...
				Status: capsulev1beta2.ResourcePermitStatus{
					...
					Request: &capsulev1beta2.ResourcePermitStatusRequest{Resources: []apiruntime.RenderedResource{{
						Targets: []runtime.RawExtension{mtConfigMapRendered},
					}}},
				},
			},
		...
		},
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseApproved, br.Status.Phase)
				approved := br.LatestTransition(capsulev1beta2.ResourcePermitPhaseApproved)
		...
		},
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseActive, br.Status.Phase)
				require.NotNil(t, br.Status.Request)
				...
				require.Len(t, br.Status.Transitions, 2)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseApproved, br.Status.Transitions[0].Type)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseActive, br.Status.Transitions[1].Type)

				obj := br.Status.Request.Resources[0].Targets[0].Object
				co, ok := obj.(client.Object)
				assert.True(t, ok)
				assert.Empty(t, co.GetOwnerReferences())
				assert.Equal(t, meta.ValueAppResourcePermitManager, co.GetLabels()[meta.AppManagedByLabel])
			},
		},
		{
			...
			verify: func(t *testing.T, br *capsulev1beta2.ResourcePermit) {
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseFailed, br.Status.Phase)
				require.NotNil(t, br.Status.Failure)
				assert.Equal(t, capsulev1beta2.ResourcePermitFailureStageActivation, br.Status.Failure.Stage)
				assert.Equal(t, capsulev1beta2.ResourcePermitPhaseApproved, br.Status.Failure.RetryPhase)
				assert.Equal(t, uint(1), br.Status.Size)
				require.Len(t, br.Status.ProcessedItems, 1)
...
func TestRecordTransitionEventOnce(t *testing.T) {
	...
	tests := []struct {
		...
		action  string
		message string
		actor   capsulev1beta2.ResourcePermitTransitionActor
	}{
		{
			name:    "review requested",
			phase:   capsulev1beta2.ResourcePermitPhaseRequested,
			reason:  evt.ReasonResourcePermitReviewNeeded,
		...
		},
		{
			name:    "approval",
			phase:   capsulev1beta2.ResourcePermitPhaseApproved,
			reason:  evt.ReasonResourcePermitApproved,
			action:  evt.ActionApproved,
			message: "Resource permit approved by alice",
			actor: capsulev1beta2.ResourcePermitTransitionActor{
				Name: "alice",
				Type: resourcepermit.AccessEntityTypeUser,
			},
		...
		{
			name:    "denial",
			phase:   capsulev1beta2.ResourcePermitPhaseDenied,
			reason:  evt.ReasonResourcePermitDenied,
			action:  evt.ActionDenied,
			message: "Resource permit denied by alice",
			actor: capsulev1beta2.ResourcePermitTransitionActor{
		...
		},
		{
			...
			reason:  evt.ReasonResourcePermitActivated,
			action:  evt.ActionActivated,
			message: "Resource permit activated",
			actor: capsulev1beta2.ResourcePermitTransitionActor{
		...
		},
		{
			...
			reason:  evt.ReasonResourcePermitExpired,
			action:  evt.ActionExpired,
			message: "Resource permit expired by alice",
			actor: capsulev1beta2.ResourcePermitTransitionActor{
				Name: "alice",
				Type: resourcepermit.AccessEntityTypeUser,
			},
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			r := &ResourcePermitReconciler{recorder: recorder}
...
func TestResourcePermitReconcilerLoadsNamespacedTemplateLocally(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	local, ok := loaded.(*capsulev1beta2.ResourcePermitTemplate)
	require.True(t, ok)
