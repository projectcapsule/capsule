...
func markRequestFailed(
	...
	stage capsulev1beta2.ResourcePermitFailureStage,
	retryPhase capsulev1beta2.ResourcePermitPhase,
	err error,
) error {
	if failureErr := br.FailPermit(stage, retryPhase, statusReason(err), err.Error()); failureErr != nil {
...
func setReconcileReady(br *capsulev1beta2.ResourcePermit, err error) {
	switch {
	case err != nil:
		br.SetReady(metav1.ConditionFalse, statusReason(err), err.Error())
	case br.Status.Phase == capsulev1beta2.ResourcePermitPhaseFailed && br.Status.Failure != nil:
		br.SetReady(metav1.ConditionFalse, br.Status.Failure.Reason, br.Status.Failure.Message)
	case br.Status.Phase == capsulev1beta2.ResourcePermitPhaseFailed:
...
func (r *ResourcePermitReconciler) SetupWithManager(mgr ctrl.Manager, _ utils.ControllerOptions) error {
	...
	r.resources = ssa.Manager{
		...
		Metadata: ssa.Metadata{
			...
			ProtectedByValue:                    meta.ValueControllerResourcePermit,
			ProtectedByServiceAccountAnnotation: meta.ResourcePermitServiceAccountAnnotation,
			AppManagedByValue:                   meta.ValueAppResourcePermitManager,
		},
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&capsulev1beta2.ResourcePermit{}).
...
func (r *ResourcePermitReconciler) Reconcile(
	...
	defer func() {
		r.Metrics.RecordResourcePermitPhase(br)
	}()
...
func (r *ResourcePermitReconciler) reconcile(
	...
	case capsulev1beta2.ResourcePermitPhasePending:
		log.V(5).Info("ResourcePermit is pending, waiting for TTL")
	...
	case capsulev1beta2.ResourcePermitPhaseApproved:
		...
		if activationErr != nil {
			return result, markRequestFailed(
				br,
				capsulev1beta2.ResourcePermitFailureStageActivation,
				capsulev1beta2.ResourcePermitPhaseApproved,
				activationErr,
			)
		}
	...
	case capsulev1beta2.ResourcePermitPhaseFailed:
		log.V(5).Info("ResourcePermit failed and is waiting for retry or expiry")
	...
	case capsulev1beta2.ResourcePermitPhaseDenied:
		...
		log.V(5).Info("ResourcePermit is denied, handling denied state")
	...
	case capsulev1beta2.ResourcePermitPhaseActive:
		...
		if err := br.ExpirePermit(nil); err != nil {
			return ctrl.Result{}, err
		}
		...
		r.recordTransitionEvent(
			...
			evt.ReasonResourcePermitExpired,
			evt.ActionExpired,
		)

		return ctrl.Result{}, nil

	// When the ResourcePermit has expired
	case capsulev1beta2.ResourcePermitPhaseExpired:
		...
		if br.Status.KeepUntil == nil ||
			...
			log.V(5).Info("ResourcePermit is expired, deleting br")
			br.DeletePermit()
		...
		log.V(5).WithValues("keep-date", br.Status.KeepUntil.Time).
			Info("ResourcePermit is expired, Holding expired state until keep date is reached")

		return ctrl.Result{RequeueAfter: time.Until(br.Status.KeepUntil.Time)}, nil

	// The case when the ResourcePermit is newly created
	case "", capsulev1beta2.ResourcePermitPhaseCreated:
...
func (r *ResourcePermitReconciler) recordTransitionEvent(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
...
func (r *ResourcePermitReconciler) reconcileApproved(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) (ctrl.Result, error) {
	log.V(5).Info("ResourcePermit is approved, checking if duration can be started")

	if br.Status.Request == nil {
		return ctrl.Result{}, fmt.Errorf("ResourcePermit is in Approved phase but status.request is nil")
	}
	...
	if err != nil {
		return ctrl.Result{}, statusError(templateResolutionFailedReason, fmt.Errorf(
			"failed to get ResourcePermit Template %s: %w",
	...
	if err := br.CheckApprovalConditions(ctx, brt); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to verify approval for ResourcePermit %s: %w", br.Name, err)
	}

	approvals := br.ApprovalPolicy(brt)
	if len(approvals.Approvers) > 0 {
		if br.Status.Review == nil || br.Status.Review.Reviewer == nil {
			return ctrl.Result{}, fmt.Errorf("ResourcePermit %s has no reviewer", br.Name)
		}

		reviewer := br.Status.Review.Reviewer
		if reviewer.Type != resourcepermit.AccessEntityTypeSystem &&
			!approvals.IsApprover(reviewer.Name, reviewer.Groups) {
			return ctrl.Result{}, fmt.Errorf("reviewer %q is not permitted to approve ResourcePermit %s", reviewer.Name, br.Name)
		}
	}
	...
	if err := br.ResolvePermitStatus(brt); err != nil {
		return ctrl.Result{}, err
	}

	if br.Status.Request.StartTime != nil {
		if wait := time.Until(br.Status.Request.StartTime.Time); wait > 0 {
			log.V(5).Info("ResourcePermit is approved, waiting for startTime", "startTime", br.Status.Request.StartTime.Time)
	...
	log.V(5).Info("ResourcePermit is approved, activating br")

	if err := r.transitionRequestActivation(ctx, br, resourceClient); err != nil {
		return ctrl.Result{}, fmt.Errorf(
			"failed to activate ResourcePermit %s: %w",
			br.Name,
			err,
		)
	}

	log.V(5).Info("ResourcePermit activated successfully")

	r.recordTransitionEvent(
		ctx,
		br,
		capsulev1beta2.ResourcePermitPhaseActive,
		evt.ReasonResourcePermitActivated,
		evt.ActionActivated,
	)
...
func (r *ResourcePermitReconciler) reconcileNew(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) (ctrl.Result, error) {
	...
	properties.Template = &capsulev1beta2.ResolvedResourcePermitTemplateReference{
		ResourcePermitTemplateReference: br.Spec.Template,
	...
	retryPhase := capsulev1beta2.ResourcePermitPhaseRequested
	autoApprove := false
	...
	if br.ApprovalPolicy(brt).Auto {
		...
		if approvalErr != nil {
			return ctrl.Result{}, fmt.Errorf(
				"auto approval could not be evaluated for ResourcePermit %s: %w",
				br.Name,
				approvalErr,
			)
		}
		...
		if approved {
			retryPhase = capsulev1beta2.ResourcePermitPhaseApproved
			autoApprove = true
		}
	}
	...
	if err := r.validateResolvedServiceAccount(ctx, br); err != nil {
		return ctrl.Result{}, markRequestFailed(
			br,
			capsulev1beta2.ResourcePermitFailureStagePreflight,
			retryPhase,
	...
	return ctrl.Result{}, br.ApprovePermit(&resourcepermit.AccessEntity{
		Type: resourcepermit.AccessEntityTypeSystem,
	}, br.Status.Request, "Auto Approved")
}

func (r *ResourcePermitReconciler) reconcileRetrying(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) (ctrl.Result, error) {
	if br.Status.Failure == nil {
		return ctrl.Result{}, errors.New("ResourcePermit is Retrying but status.failure is nil")
	}

	failure := *br.Status.Failure

	switch failure.Stage {
	case capsulev1beta2.ResourcePermitFailureStagePreflight:
	...
	case capsulev1beta2.ResourcePermitFailureStageActivation:
...
func (r *ResourcePermitReconciler) loadTemplate(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
	...
	case capsulev1beta2.GlobalResourcePermitTemplateKind:
		...
		return brt, nil
	default:
		return nil, fmt.Errorf("unsupported ResourcePermit template kind %q", br.Spec.Template.Kind)
	}
}

func (r *ResourcePermitReconciler) renderResources(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
	brt capsulev1beta2.ResourcePermitTemplateSource,
	...
	for resourceIndex := range rendered {
		for targetIndex, raw := range rendered[resourceIndex].Targets {
			...
			labels[meta.AppManagedByLabel] = meta.ValueAppResourcePermitManager
...
func readyMessage(br *capsulev1beta2.ResourcePermit) string {
	switch br.Status.Phase {
	case capsulev1beta2.ResourcePermitPhaseCreated:
...
func (r *ResourcePermitReconciler) requestReview(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) (ctrl.Result, error) {
	log.V(5).Info("ResourcePermit is ready for review")

	r.recordTransitionEvent(
		ctx,
		br,
		capsulev1beta2.ResourcePermitPhaseRequested,
		evt.ReasonResourcePermitReviewNeeded,
...
func (r *ResourcePermitReconciler) updateStatus(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) func() {
...
// ResourcePermit can be retained for its configured audit period.
func (r *ResourcePermitReconciler) addFinalizer(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
) error {
...
func (r *ResourcePermitReconciler) namespaceTerminating(
	ctx context.Context,
	namespace string,
) (bool, error) {
	if namespace == "" {
...
func (r *ResourcePermitReconciler) transitionRequestActivation(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
	resourceClient client.Client,
) error {
	...
	if err := brCopy.ActivatePermit(nil); err != nil {
		return err
	}
	...
	if err := r.reconcileItems(ctx, brCopy, resourceClient); err != nil {
		...
		return statusError(resourceApplyFailedReason, fmt.Errorf(
			"failed to create ResourcePermit items %s: %w",
			brCopy.Name,
			err,
		))
	}
...
func (r *ResourcePermitReconciler) dryRunItems(
	...
	manager := r.managedResourceManager(resourceClient, br)
	fieldOwner := meta.ResourcePermitFieldOwner(br)
...
// Creates the necessary items resources for the ResourcePermit.
func (r *ResourcePermitReconciler) reconcileItems(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
	resourceClient client.Client,
) (err error) {
	...
	for resourceIndex := range br.Status.Request.Resources {
		...
		for targetIndex, raw := range resource.Targets {
			...
			if br.Status.Active.ActiveUntil != nil {
				...
				ann[meta.ResourcePermitActiveUntilAnnotation] = br.Status.Active.ActiveUntil.Format(time.RFC3339)
			...
			// ResourcePermits are namespaced but may manage cluster-scoped objects,
			// so their lifecycle cannot rely on Kubernetes owner references. The
			// ResourcePermit finalizer and recorded target identities provide the
...
// pruneItems relinquishes the ResourcePermit field manager's resources.
func (r *ResourcePermitReconciler) pruneItems(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
	resourceClient client.Client,
) (err error) {
...
func (r *ResourcePermitReconciler) validateResolvedServiceAccount(
	ctx context.Context,
	br *capsulev1beta2.ResourcePermit,
) error {
...
func (r *ResourcePermitReconciler) resourceClient(
	ctx context.Context,
	log logr.Logger,
	br *capsulev1beta2.ResourcePermit,
	brt capsulev1beta2.ResourcePermitTemplateSource,
	...
	if br.Status.Request == nil {
		br.Status.Request = &capsulev1beta2.ResourcePermitStatusRequest{}
	}
	...
	log.V(5).Info(
		"using impersonation client for ResourcePermit resources",
...
func (r *ResourcePermitReconciler) resolveTemplateServiceAccount(
	log logr.Logger,
	brt capsulev1beta2.ResourcePermitTemplateSource,
...
func (r *ResourcePermitReconciler) resolveGlobalTemplateServiceAccount(
	log logr.Logger,
	brt *capsulev1beta2.GlobalResourcePermitTemplate,
) *meta.NamespacedRFC1123ObjectReferenceWithNamespace {
...
func (r *ResourcePermitReconciler) managedResourceManager(
	resourceClient client.Client,
	br *capsulev1beta2.ResourcePermit,
) ssa.Manager {
	manager := r.resources
	...
	if manager.Metadata.CreatedByValue == "" {
		manager.Metadata.CreatedByValue = meta.ValueControllerResourcePermit
	}

	if manager.Metadata.ManagedByValue == "" {
		manager.Metadata.ManagedByValue = meta.ValueControllerResourcePermit
	}

	if manager.Metadata.ProtectedByValue == "" {
		manager.Metadata.ProtectedByValue = meta.ValueControllerResourcePermit
	}
	...
	if manager.Metadata.AppManagedByValue == "" {
		manager.Metadata.AppManagedByValue = meta.ValueAppResourcePermitManager
	}
