...
func (r *clusterCustomQuotaClaimController) OnCreate(e event.TypedCreateEvent[client.Object]) bool {
	//nolint:forcetypeassert
	cq := e.Object.(*capsulev1beta2.ClusterCustomQuota)

	cq.Status = capsulev1beta2.CustomQuotaStatus{}

	namespaces, err := GetNamespacesMatchingSelectors(context.Background(), cq.Spec.Selectors, r.Client)
	if err != nil {
...
func (r *clusterCustomQuotaClaimController) OnUpdate(e event.TypedUpdateEvent[client.Object]) bool {
	//nolint:forcetypeassert
	...
	if !equality.Semantic.DeepEqual(customQuotaOld.Spec.ScopeSelectors, customQuotaNew.Spec.ScopeSelectors) ||
		...
		namespaces, err := GetNamespacesMatchingSelectors(context.Background(), customQuotaNew.Spec.Selectors, r.Client)
		if err != nil {
			r.log.Error(err, "Error getting namespaces while updating CustomQuota usage")
