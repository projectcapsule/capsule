...
func TestGlobalResourceLeaseTemplateReconciler(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			selectors: []selectors.NamespaceSelector{{
				LabelSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"resource-lease": "enabled"}},
			}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			template := &capsulev1beta2.GlobalResourceLeaseTemplate{
			...
			cl := fake.NewClientBuilder().
				WithScheme(scheme).
				WithStatusSubresource(&capsulev1beta2.GlobalResourceLeaseTemplate{}).
				WithObjects(
					template,
					&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "team-a", Labels: map[string]string{"resource-lease": "enabled"}}},
					&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "team-b"}},
					&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "team-c", Labels: map[string]string{"resource-lease": "enabled"}}},
				).
				Build()

			r := &GlobalResourceLeaseTemplateReconciler{Client: cl}
			...
			got := &capsulev1beta2.GlobalResourceLeaseTemplate{}
...
func TestGlobalResourceLeaseTemplateStatusNamespacePresent(t *testing.T) {
	t.Parallel()

	selected := capsulev1beta2.GlobalResourceLeaseTemplateStatus{Namespaces: []string{"team-a"}}
	...
	unrestricted := capsulev1beta2.GlobalResourceLeaseTemplateStatus{Namespaces: []string{"*"}}
