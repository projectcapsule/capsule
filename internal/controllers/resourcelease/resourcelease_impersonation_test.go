...
func TestResolveTemplateServiceAccount(t *testing.T) {
	...
	r := &ResourceLeaseReconciler{Configuration: impersonationTestConfiguration{properties: configured}}
	...
	brt := &capsulev1beta2.GlobalResourceLeaseTemplate{
		Spec: capsulev1beta2.GlobalResourceLeaseTemplateSpec{Impersonation: explicit},
	}
...
func TestResolveTemplateServiceAccountWithoutCompleteDefault(t *testing.T) {
	...
	for _, properties := range []capsulev1beta2.ServiceAccountClient{
		...
		r := &ResourceLeaseReconciler{Configuration: impersonationTestConfiguration{properties: properties}}
		if got := r.resolveTemplateServiceAccount(logr.Discard(), &capsulev1beta2.GlobalResourceLeaseTemplate{}); got != nil {
...
func TestResourceClientPinsControllerIdentityWithoutImpersonation(t *testing.T) {
	...
	base := fake.NewClientBuilder().Build()
	r := &ResourceLeaseReconciler{Client: base}

	for _, tt := range []struct {
		name     string
		template *capsulev1beta2.GlobalResourceLeaseTemplate
	}{
		{name: "while resolving a template", template: &capsulev1beta2.GlobalResourceLeaseTemplate{}},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if br.Status.Request.Impersonation == nil {
				t.Fatal("controller ServiceAccount was not posted to ResourceLease status")
			}
