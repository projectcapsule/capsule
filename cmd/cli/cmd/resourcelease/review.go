...
var reviewCmd = &cobra.Command{
	Use:   "review",
	Short: "Review a ResourceLease",
	Args:  cobra.ExactArgs(1),
	Example: `
  # interactive review
  kubectl capsule resource-lease review grant-admin --namespace default

  # non-interactive approve/deny
  kubectl capsule resource-lease review grant-admin --namespace default --approve
  kubectl capsule resource-lease review grant-admin --namespace default --deny
...
`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if br.Status.Phase == "" {
			return fmt.Errorf(
				"ResourceLease %s is not yet processed, current phase: %q",
				name,
				br.Status.Phase,
			)
		}

		if br.Status.Phase != v1beta2.ResourceLeasePhaseRequested {
			return fmt.Errorf(
				"ResourceLease %s is not in Requested phase (already reviewed), current phase: %q",
				name,
				br.Status.Phase,
			)
		}
		...
		if br.Status.Request == nil {
			return fmt.Errorf("ResourceLease %s has no prepared request properties", name)
		}
		...
		if keepForStr != "" {
			...
			keepFor := resourcelease.ExtendedDuration(d)
		...
		if approveFlag {
			...
			printResourceLeasesApprovalTable(br, props, !noColorFlag)
		...
		return retry.OnError(
			...
			func() error {
				...
				return patchResourceLeaseStatus(ctx, k8sClient, br, func() error {
					switch action {
					case approveValue:
						br.Status.Phase = v1beta2.ResourceLeasePhaseApproved
						br.Status.Request = props.DeepCopy()
					case denyValue:
						br.Status.Phase = v1beta2.ResourceLeasePhaseDenied
					default:
