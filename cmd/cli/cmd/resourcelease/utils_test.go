...
func TestPatchResourceLeaseStatusPreservesControllerManagedFields(t *testing.T) {
	t.Parallel()
	...
	approvals := &resourceleaseapi.ApprovalSpec{Conditions: []string{`requestor.name == "alice"`}}
	stored := &capsulev1beta2.ResourceLease{
	...
	require.NoError(t, patchResourceLeaseStatus(ctx, k8sClient, partial, func() error {
		partial.Status.Phase = capsulev1beta2.ResourceLeasePhaseExpired
	...
	assert.Equal(t, capsulev1beta2.ResourceLeasePhaseExpired, current.Status.Phase)
