	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&capsulev1beta2.ResourcePoolClaim{},
			handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []reconcile.Request {
				...
				claim, ok := obj.(*capsulev1beta2.ResourcePoolClaim)
				if !ok {
					return nil
				}
...
func (r resourcePoolController) Reconcile(ctx context.Context, request ctrl.Request) (result ctrl.Result, err error) {
	...
	if err = r.Get(ctx, request.NamespacedName, instance); err != nil {
		if apierrors.IsNotFound(err) {
			...
			metrics.ResourcePoolQueueSize.DeletePartialMatch(map[string]string{"pool": request.Name})
			metrics.ResourcePoolLimit.DeletePartialMatch(map[string]string{"pool": request.Name})
			metrics.ResourcePoolLimit.DeletePartialMatch(map[string]string{"pool": request.Name})
...
func (r *resourcePoolController) reconcile(
	...
	// Garbage collect orphaned ResourceQuotas
	r.garbageCollection(ctx, log, pool, allClaims, namespaces)

	// After Garbage Collection we can update the new namespaces
	...
	for _, claim := range allClaims {
		...
		err = r.reconcileResourceClaim(ctx, log.WithValues("Claim", claim.Name), pool, &claim)
		if err != nil {
...
func (r *resourcePoolController) reconcileResourceClaim(
	...
	errs := pool.CanClaimWithinNamespace(log, claim)
	if len(errs) > 0 {
...
func (r *resourcePoolController) bindClaimToPool(
	...
	cond.Message = fmt.Sprintf("Claimed resources")
...
func (r *resourcePoolController) garbageCollection(
	...
	for ns, clms := range pool.Status.Claims {
		...
		for _, cl := range clms {
			...
			log.V(5).Info("GARBAGE COLLECT", "claims", claimSet, "uid", cl.UID)
