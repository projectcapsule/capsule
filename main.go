...
func main() {
	...
	webhooksList := append(
		...
		route.NamespaceValidate(utils.InCapsuleGroups(cfg, namespacewebhook.QuotaHandler(), namespacewebhook.FreezeHandler(cfg), namespacewebhook.PrefixHandler(cfg), namespacewebhook.UserMetadataHandler())),
		route.NamespacePatch(utils.InCapsuleGroups(cfg, namespacewebhook.PatchHandler(cfg))),
